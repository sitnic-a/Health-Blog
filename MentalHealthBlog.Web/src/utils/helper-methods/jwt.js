...
export const checkIfTokenIsExpired = (token) => {
  ...
  let tokenShouldBeRenewed = false

  // The most basic way of renewing users jwtoken and authentication in application
  ...
  if (!stringIsNullOrEmpty(token)) {
    jwtIntervalId = setInterval(() => {
      ...
      if (difference <= TEN_SECONDS) {
        ...
        tokenShouldBeRenewed = true
        localStorage.setItem('tokenShouldBeRenewed', tokenShouldBeRenewed)
        console.log('Token is being renewed...')
      }
    }, 3000)
  }
  return
